...
- **Production is forced off: positive `ENVIRONMENT` allowlist, not a negated production test** (amended 2026-09-26, security fix found by the CE). The live production Worker deploys from the **top-level** configuration (`--env=""`), where `ENVIRONMENT` is unset, so an `ENVIRONMENT === "production"` check would never fire and a `!= "production"` test would wrongly enable the mock in production. Rule:
  - The mock provider path is eligible only if `ENVIRONMENT` **exactly** matches a value on the mock-eligible environment allowlist `NON_PRODUCTION_ENVIRONMENTS = ["staging"]` (a positive list in `src/flags.ts`; today only `"staging"`). Adding another value, for example `development` for local `wrangler dev`, is a reviewed code change.
