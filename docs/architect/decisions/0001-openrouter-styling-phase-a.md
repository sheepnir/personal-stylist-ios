...
1. **Mocked provider only.** The provider is an injected interface (`StylistProvider`) with a mock implementation. **Production (live top-level Worker):** the flag is off and the provider path is unreachable (section 9). The mock runs only when `ENVIRONMENT` is **exactly** one value on the **mock-eligible environment allowlist** (today only `staging`; section 9) and `PROVIDER_GENERATION=mock`, with all other eligibility checks passing. Independently, a **phase A guard in the Worker blocks `callOpenRouter` (and any fetch to the OpenRouter host) in every environment**, so phase A makes **zero OpenRouter network calls** regardless of flag. A test fails if the real client is invoked while phase A is in force.
...
3. **Flag off unless allowlisted.** The mock provider path is off unless the flag and every eligibility check pass (section 9). Production keeps the flag off in phase A, enforced in code and not only by configuration. If `ENVIRONMENT` is unset, empty, unknown, or not an exact allowlist match, the mock path is **off** (never inferred by negating `production`).
...
| Flag | Worker environment variable `PROVIDER_GENERATION`, parsed fail-closed and off by default. Mock eligibility requires `ENVIRONMENT` to **exactly** match a value on the mock-eligible environment allowlist (today only `staging`; unset, empty, or unknown means off). Plus an optional per-environment device allowlist and config guards. Section 9. |
...
The Worker builds the provider payload **after** Stage 1 and Stage 2 have run. The model sees only the shortlist (openapi design rule 2, D-03). Garment UUIDs are replaced with per-request random tokens (`g_` + 4 base-36 characters, not derived from the UUID). Sets get `s_` tokens. The request builder **must regenerate any garment or set token that collides within the same request** until all tokens in that request are unique. The Worker keeps the token→UUID map in memory for the request only.
...
6. **Policy version (PM decision (b) and the final PM decision, 2026-09-26).**
   ...
   - **Acceptance rule:** accepted **if and only if** the served version is a **non-empty string** **and** the stored accepted version equals it exactly (case-sensitive, no trimming). So `null` never matches anything, including an empty or missing stored value. While the served version is `null`, no device is accepted and the stylist-model path stays off (fail-closed), **including the phase A mock**, until #11 publishes a real policy. That is separate from the flag: production keeps the flag off; the mock may run only on an allowlisted `ENVIRONMENT` (today `staging`) with `PROVIDER_GENERATION=mock` when acceptance and the other eligibility checks pass (section 9). A not-accepted request takes today's deterministic path with no `fallbackReason` (consent not given is the product working as configured, like flag-off). The app applies the same rule to decide whether to prompt (#11). The Worker enforces it independently, because a client-side check isn't a control.
...
- **Evaluation model (founder, 2026-09-26):** `typesafe/jev-1.13` is the chosen model for design, eval, and eventual live use (section 7.1). **Production activation** of that model stays gated on every paid-activation precondition in section 11 (including a founder-authorized application budget and an overall application spending cap). PRD §10.1's three-candidate comparison procedure is not re-run for this founder choice; departures are recorded in section 7.1.6. The first-comparison budget is internal D-09, Proposed, not authorized.
...
- **Live allowlist entry:** added only at paid activation (section 7.1, section 11), by a PR that records slug, prices, and check date. **Spend implications:** every allowlisted model's price feeds the upper-bound reservation. A frontier model raises the per-attempt reservation and so reaches the per-device and global caps sooner. **Soft threshold (D-20; CTO, 2026-09-26):** before **every** eligible provider attempt, the Worker evaluates the soft threshold against current ledger totals (**enforced before calls**, satisfying #26). When crossed, the response surfaces a **soft-threshold state to the client** (for example via `spendState` / usage summary fields the contract already exposes). It does **not** block the attempt and does **not** switch models; only the **hard cap** refuses the reservation and triggers deterministic fallback with `fallbackReason: "SPEND_CAP"`.
...
- **Phase A (typed-decision / Decisions shape):** the version name is **`outfit-t2-d1`**. The immutable module lives in the engine package (pure, no network), for example `backend/outfit-engine/src/provider/prompts/outfit-t2-d1.ts`. It exports the fixed **`instructionText`**, the schema of **`optionDescriptions`**, and **`answerTypes`**. There is **no** chat `system` prompt and **no** `outputSchema` in phase A. **`buildRequest(payload)`** is request-builder code delivered in slice **A-3**; it is **not** part of the hashed prompt content.
- A registry test hashes (SHA-256) the **stable JSON** of `instructionText`, `optionDescriptions`, and `answerTypes` (**excluding** the version string) and compares it to the hash registered for that version. Editing hashed content without adding a new version fails CI. Old versions are never edited or deleted while stored outfits reference them. *(A future chat-completion prompt module may hash `system` plus serialized `outputSchema` instead; that is out of phase A.)*
...
- **Production is forced off: positive `ENVIRONMENT` allowlist, not a negated production test** (amended 2026-09-26, security fix found by the CE). The live production Worker deploys from the **top-level** configuration (`--env=""`), where `ENVIRONMENT` is unset, so an `ENVIRONMENT === "production"` check would never fire and a `!= "production"` test would wrongly enable the mock in production. **Rejected (do not use for eligibility):** inferring mock access from `ENVIRONMENT != "production"`, describing the list as **non-production**, or a `NON_PRODUCTION_*` constant name—production's unset `ENVIRONMENT` would pass those tests. Rule:
  - The mock provider path is eligible only if `ENVIRONMENT` **exactly** matches a value on `MOCK_ELIGIBLE_ENVIRONMENTS = ["staging"]` (positive list in `src/flags.ts`; today only `"staging"`). Adding another value, for example `development` for local `wrangler dev`, is a reviewed code change.
  - Unset, empty, `production`, any other value, or a different case is **not** on that allowlist, so the mock path is **off**. `PROVIDER_GENERATION = "mock"` on its own is never enough.
...
- **Eligibility, all required:** flag resolves to `mock`, `ENVIRONMENT` **exactly** matches a value on the mock-eligible environment allowlist (today only `staging`), the token is per-device (not legacy), the device is on the allowlist when one is set, the request's accepted policy version equals the served version (section 5.6 rule), and a primary model is configured and allowlisted. If the flag is off, `ENVIRONMENT` is not allowlisted, the policy version isn't accepted, or the device isn't eligible, the request takes today's exact deterministic path: no ledger access, no provider call, no `fallbackReason`.
...
- **Turning it on later:** (a) **Mock on an allowlisted environment only:** set `PROVIDER_GENERATION=mock` where `ENVIRONMENT` is exactly an allowlisted value (today deploy to `[env.staging]` only) when policy acceptance and the other eligibility checks pass. Production (top-level deploy, `ENVIRONMENT` unset) keeps the flag off and the path unreachable. Phase A still blocks `callOpenRouter` everywhere (zero OpenRouter calls). (b) **Live provider:** requires a new code change adding a `live` flag value, **all** section 11 preconditions (founder-authorized application budget, overall application spending cap, #12–#14, verified provider settings, logging redaction, disclosure, release decision), then deploy. A production flag change counts as a release per DevFlow draft SOP-010.
...
- **Type (contract):** an **extensible string**. OpenAPI must **not** use a closed `enum`. The **`fallbackReason` OpenAPI `description`** documents the known `UPPER_SNAKE_CASE` values below and **directs clients to treat any unknown value as a generic fallback** (same user-facing notice as an eligible-path fallback, not a stylist-model result). New values may be added without a major API version. **`fallbackCause`** stays server-only and is **never** sent to clients (section 10.2).
- **Known values (minimal set, UPPER_SNAKE_CASE like `FallbackLevel`, `spendState`, and Problem `code`):**
...
- **Client rule for unknown values:** decode as `String`. Follow the OpenAPI `description`: show the **generic fallback** notice for any unknown value; never fail decoding. The app may map unknown values to the same copy as `PROVIDER_ERROR`, but the contract requirement is generic fallback, not a closed enum.
- **Internal cause code (server-only, never sent to clients):** every fallback also sets `fallbackCause`, which is written only to Worker logs and diagnostics (the structured log line and the eval harness output). Authoritative values:
  ...
  - under `INVALID_OUTPUT`: `OUTPUT_PARSE`, `OUTPUT_SCHEMA`, `OUTPUT_MODEL_MISMATCH`, `OUTPUT_TOKEN_MAP`, `OUTPUT_STAGE4` (with violation codes), `OUTPUT_EXCLUDED_SET`, `OUTPUT_TEXT`, `OUTPUT_LENGTH`
...
```yaml
        fallbackReason:
          ...
          description: |
            ...
            model path was offered (for example, provider generation disabled). Extensible
            string (no closed enum). Known values today: `PROVIDER_ERROR` — the provider
            failed, timed out, was rate limited, or the configured model is not allowed;
            `INVALID_OUTPUT` — the provider answered but the output failed validation;
            `SPEND_CAP` — a spend cap refused the paid attempt (see `spendState`). Clients
            MUST treat any unknown value as a **generic fallback** (show the generic fallback
            notice, not a stylist-model result) and MUST NOT fail decoding; new values may be
            added without a major version.
```
...
- **Lookup schedule (#13-b; source of truth is `backend/workers/src/ledgerCore.ts`, constants `COST_LOOKUP_BACKOFF_MS` and `UNKNOWN_OUTCOME_AGING_MS`).** Each interval is the minimum gap between one cost lookup for an attempt and the next. Lookups run only when that device's ledger is accessed, so an actual gap can be longer, never shorter.

  | Lookup | Minimum gap after the previous lookup (the first counts from the reservation) |
  |---|---|
  | 1 | 0 (allowed at the first ledger access) |
  | 2 | 1 min |
  | 3 | 5 min |
  | 4 | 15 min |
  | 5 | 1 h |
  | 6 and later | 4 h, repeating until the cost is known or the attempt ages |

  - **Aging:** 24 h after the reservation, an attempt whose cost is still unknown counts as spent at its reserved upper bound, on the ledger day of the reservation.
  - **No generation id:** no lookup is attempted; the attempt ages at 24 h.
  - **Unknown or error results never release the reservation.** Only a known cost reconciles it, releasing the difference between the reservation and the known cost.
  - **Source of truth:** the constants in code (`COST_LOOKUP_BACKOFF_MS`, `UNKNOWN_OUTCOME_AGING_MS`). Tuning them needs **no ADR amendment**, but **this table must be updated in the same PR** that changes the constants. Changing the aging period or the rule that unknown or error results never release funds requires an ADR amendment.
...
| 2 | How the Worker learns the stored accepted policy version on image-free generate (recommended: optional `GenerateRequest.acceptedPolicyVersion`, section 5.6). Until #11 publishes a policy and this carrier is decided, the stylist-model path (mock included) stays off for lack of acceptance; mock enablement on an allowlisted `ENVIRONMENT` still requires acceptance once a policy exists. | CTO (PM consulted, #11 owner) |
...
- **Harder:** rationales use no free-text or profile context in phase A (PM decisions (a) and (e)), so eval quality with the mock and early live runs understates what profile inputs could add. While `policyVersion` is served as `null`, acceptance blocks the stylist-model path (mock included); production also keeps the flag off. The mock may be exercised on an allowlisted `ENVIRONMENT` (today `staging`) after #11 and the section 5.6 carrier land. Paid activation still needs section 11 (budget, overall cap, and the rest).
...
- **Accepted** as written, with redactions for the public repo: the first-comparison budget appears only as "internal D-09, Proposed, not authorized". The founder-chosen evaluation model `typesafe/jev-1.13` is named in section 7.1; live activation remains gated on section 11. This record contains no dollar figures or cap values. (`mock/stylist-v0` is a mock identifier, not a provider slug.)
...
- 2026-09-26: Section 9 security amendment (found by the CE). The live production deploy uses the top-level config, where `ENVIRONMENT` is unset, so the `ENVIRONMENT === "production"` force-off was inert. Replaced with positive allowlist `MOCK_ELIGIBLE_ENVIRONMENTS = ["staging"]`: unset or unknown is not allowlisted, so the mock path is off. Added the required environment tests, `scripts/check-provider-flag-off.py` (Repo Checks plus CE pre-deploy strict mode), a post-deploy `wrangler versions view --json` confirmation, and explicit "off" in every live config block (private). No flag field on `/health`. This is the first read of `ENVIRONMENT` in the Worker.
...
- 2026-09-26: Section 12 records the #13-b cost-lookup schedule from `ledgerCore.ts` at commit `00630c9` (lookup gaps of 0, 1 min, 5 min, 15 min, and 1 h, then every 4 h until known or aged; aging at 24 h; no lookup without a generation id; unknown or error never releases). It holds no spend, cap, or account figures, so it is recorded publicly (Architect ruling, 2026-09-26). The code constants remain the source of truth.
- 2026-09-26: CTO rulings from GPT-5.6 Sol review @ `d5457cb` (REQUEST CHANGES, no blockers): `fallbackReason` is an extensible OpenAPI string (known values documented; no closed enum); `OUTPUT_MODEL_MISMATCH` added to the authoritative `fallbackCause` list; mock path clarified (production flag off; mock only when `ENVIRONMENT` is exactly `staging` on `MOCK_ELIGIBLE_ENVIRONMENTS`; phase A blocks `callOpenRouter` everywhere); soft threshold evaluated before every call with client-visible state and only the hard cap blocks; phase A prompt hash covers `instructionText`, `optionDescriptions`, and `answerTypes` (`outfit-t2-d1`; `buildRequest` not hashed); section 7 records the founder-chosen evaluation model and removes contradictory "none approved" text; token collision regeneration; section 12 table tracks code constants (tune without ADR, table updated in the same PR).
