...
1. **Mocked provider only.** The provider is an injected interface (`StylistProvider`) with a mock implementation. **Production:** the flag is off and the provider path is unreachable (section 9). **Non-production** (for example `staging`) may enable the mock with `PROVIDER_GENERATION=mock` when all other eligibility checks pass. Independently, a **phase A guard in the Worker blocks `callOpenRouter` (and any fetch to the OpenRouter host) in every environment**, so phase A makes **zero OpenRouter network calls** regardless of flag. A test fails if the real client is invoked while phase A is in force.
...
6. **Policy version (PM decision (b) and the final PM decision, 2026-09-26).**
   ...
   - **Acceptance rule:** accepted **if and only if** the served version is a **non-empty string** **and** the stored accepted version equals it exactly (case-sensitive, no trimming). So `null` never matches anything, including an empty or missing stored value. While the served version is `null`, no device is accepted and the stylist-model path stays off (fail-closed), **including the phase A mock**, until #11 publishes a real policy. That is separate from the flag: **production** keeps the flag off and the path unreachable; **non-production** may turn the mock on with the flag only when acceptance and the other eligibility checks pass (section 9). A not-accepted request takes today's deterministic path with no `fallbackReason` (consent not given is the product working as configured, like flag-off). The app applies the same rule to decide whether to prompt (#11). The Worker enforces it independently, because a client-side check isn't a control.
...
- **Type (contract):** an **extensible string**. OpenAPI must **not** use a closed `enum`. The server sends one of the known values below today; new values may be added without a major API version.
...
- **Client rule for unknown values:** decode as `String`. Treat any value other than the three known strings like **`PROVIDER_ERROR`** (generic "built without the stylist model" notice). Never fail decoding on an unknown value.
...
```yaml
        fallbackReason:
          ...
          description: |
            ...
            MUST treat any other value like `PROVIDER_ERROR` and MUST NOT fail decoding on
            unknown values; new values may be added without a major version.
```
...
| 2 | How the Worker learns the stored accepted policy version on image-free generate (recommended: optional `GenerateRequest.acceptedPolicyVersion`, section 5.6). Until #11 publishes a policy and this carrier is decided, the stylist-model path (mock included) stays off for lack of acceptance; non-production mock enablement still requires acceptance once a policy exists. | CTO (PM consulted, #11 owner) |
...
- **Harder:** rationales use no free-text or profile context in phase A (PM decisions (a) and (e)), so eval quality with the mock and early live runs understates what profile inputs could add. While `policyVersion` is served as `null`, acceptance blocks the stylist-model path (mock included); production also keeps the flag off. Non-production may exercise the mock after #11 and the section 5.6 carrier land. Paid activation still needs section 11 (budget, overall cap, and the rest).
